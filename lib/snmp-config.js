...
const SECRET_FIELDS = ["snmp_community", "snmp_auth_key", "snmp_priv_key"];

// Encrypt secret columns before persisting a device row.
export function encryptDeviceSecrets(o) {
  const c = { ...o };
  for (const f of SECRET_FIELDS) if (c[f] != null) c[f] = encryptSecret(c[f]);
  return c;
}

// Decrypt secret columns after loading a device row (for polling / the edit form).
export function decryptDeviceRow(row) {
  if (!row) return row;
  const c = { ...row };
  for (const f of SECRET_FIELDS) if (c[f] != null) c[f] = decryptSecret(c[f]);
  return c;
}

// Remove secret columns entirely (for list responses that never need them).
export function stripDeviceSecrets(row) {
  if (!row) return row;
  const c = { ...row };
