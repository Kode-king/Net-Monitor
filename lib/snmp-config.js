...
export function validateSnmp(cfg) {
  if (cfg.snmp_version !== "3") return null;
  if (!cfg.snmp_sec_name) return "اسم المستخدم (Security Name) مطلوب لـ SNMPv3";
  if (cfg.snmp_sec_level !== "noAuthNoPriv" && !cfg.snmp_auth_key)
    return "مفتاح المصادقة (Auth Key) مطلوب";
  if (cfg.snmp_sec_level === "authPriv" && !cfg.snmp_priv_key)
    return "مفتاح التشفير (Priv Key) مطلوب";
  return null;
}
