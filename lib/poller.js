...
function tick() {
  ...
  let devices;
  try {
    devices = db.prepare("SELECT * FROM devices WHERE enabled = 1").all();
  } catch (e) {
  ...
  for (const rule of rules) {
    ...
    if (conditionMet) {
      ...
      if (held >= (rule.duration_s || 0) && !existing) {
        db.prepare(
          `INSERT INTO alerts (device_id, rule_id, metric, message, value, state, started_at)
           VALUES (?, ?, ?, ?, ?, 'firing', ?)`
        ).run(device.id, rule.id, rule.metric, `${device.name}: ${label}`, value, ts);
        console.log(`[alert] FIRING ${device.name} ${label}`);
      }
    } else {
