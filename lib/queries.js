...
import { decryptDeviceRow, stripDeviceSecrets } from "./snmp-config.js";
...
export function listDevicesWithStatus() {
  ...
  return devices.map((raw) => {
    const d = stripDeviceSecrets(raw);
    const last = db
...
export function getDevice(id) {
  return decryptDeviceRow(db.prepare("SELECT * FROM devices WHERE id = ?").get(id));
}
